...
int main(int argc, char *argv[])
{
    ...
    const char *uri = "Long.Updator";
    qmlRegisterType<UpdatorHelper>(uri, 1, 0, "Updator");
    qmlRegisterType<UpgradeableModel>(uri, 1, 0, "UpgradeableModel");

    if (!QDBusConnection::sessionBus().registerService("com.long.UpdatorGui")) {
        return 0;
    }

    // Translations
    QLocale locale;
    QString qmFilePath = QString("%1/%2.qm").arg("/usr/share/long-updator/translations/").arg(locale.name());
